...
- `pvc2609_preopen_review_publish.py`: despite the legacy filename, cron now runs the three review+forecast reports 10 minutes after the reviewed session closes: 11:40 for上午复盘+午盘预测, 15:10 for日盘复盘+夜盘预测, and 23:10 for夜盘复盘+次日日盘预测. For the 23:10 morning target, the default target date is derived from the latest completed night-session K-line and the next available day-session date, not simply the current natural date.
