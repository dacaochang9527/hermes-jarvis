...
1. 离线扫描：日线数据 + 观察池 + Markdown 日报；
2. 回测验证：历史样本、胜率、平均收益、回撤、盈亏比、连续亏损；
3. 盘中提醒：只监控昨日观察/自选股，3–5 分钟轮询；
   - 若用户要求“实时监控并通知”，优先落成 `cronjob(no_agent=True)` + 脚本 watchdog：stdout 有内容才投递，静默无信号；脚本内维护去重状态，避免微信刷屏。
   - 若用户要求“建议买入时机”，不要输出确定性买卖指令；用“观察买入时机 / 低吸观察区 / 小仓试错 / 失效位 / 不追高”等语言，把买点表达为可复盘的观察条件。
...
4. 收盘复盘：定时生成报告，汇总每只观察标的的触发事件、监控内高低点、观察价/失效位、低吸观察区、次日处理建议。
...
当用户问“D3 观察 / HOLD 持仓是否在监控池”“列一下当前监控池”时，不要只读日报观察文件或凭记忆回答。必须先核对：
...
5. 若观察文件未被监控任务读取，要明确说“观察标的存在，但当前不在实际监控池”，并给出需要替换/合并监控池的下一步。

如果发现实际监控池混入 20cm 标的，先标为“应剔除”，不要继续把它当作可执行观察标的。

### 替换监控池时必须同步替换指标

当用户要求把某个观察池（例如 `0527D3_candidate_AND.csv`）切入盘中提醒时，不要只复制 `code/name/trigger_price/invalid_price`。必须同时处理：

1. **保留观察文件里的指标字段**：至少保留 `industry`、`zone_low`、`zone_high`、`score`、`rank`、`note`；否则低吸观察区等指标会被脚本用旧公式或默认公式重新计算，看起来像“指标还是旧的”。
...
常见漏改：只替换监控 CSV，但 `scripts/tulong/runtime/review.py` 仍 import 静态 `WATCHLIST`；只写观察价/失效位，但丢失 `zone_low/zone_high`；只看观察文件，以为已经入池，却没有确认 cron 实际读取的 CSV。

### Reference 文件分工

本 skill 的 reference 只在这里维护分工说明，避免各文件重复解释关系：

- `references/tulong-selection-and-monitoring.md`：**当前主规则**。维护屠龙 D3 选股的日期+D几、D3观察区、HOLD 持仓、提醒指标、频率、事件优先、监控池替换验证。后续确认的新 D3 选股/HOLD 持仓规则优先更新这里。
...
摘要：D3 观察区 = 低吸可执行性 AND 强势潜质；D3 看买点区、回收观察价、止损；买入后即转为 `HOLD` 持仓（无日期前缀，按 `entry_date` 派生可卖性），验证期不预设买入后卖出/加仓策略（已移除，待验证后另定）；事件检测 5 分钟，快照 15 分钟，事件优先，快照被事件挤掉后约 3 分钟补发。
