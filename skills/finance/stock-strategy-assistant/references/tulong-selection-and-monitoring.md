...
D3 候选区只放当天 D3。前一交易日已经是 D3 的未买入观察票，次日不继续留在 D3；只有买入的持仓才转为 `HOLD`，并按 `HOLD_position_*` 最新快照滚动续期。
...
这些判断属于生成 `0527D3候选区` 的 D1/D2 过滤规则，不属于 HOLD 持仓的监控指标。
...
```text
15:10–15:30  收盘复盘：生成 D3 表现、HOLD 持仓续期、次日新 D3 候选
