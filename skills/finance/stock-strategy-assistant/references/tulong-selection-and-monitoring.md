...
> 验证期策略候选（0601-0602D3）：连续复盘显示，D3 可能需要在低吸观察池之外增加“强势延续雷达”，用于记录 D1 全量底池中未入 D3 watch 但 D3 当日强势延续的漏选样本。该口径暂不写入主规则细节，具体策略、触发候选和后续优化记录维护在 `reports/reviews/tulong_d3_strategy_reviews/0601_0602D3_common_strategy_conclusion_20260602.md`。
...
验证期策略候选的落盘纪律：单日或少量样本得出的具体策略候选，不要过早写入主规则细节。应优先落到 `reports/reviews/tulong_d3_strategy_reviews/` 中的独立结论文件；`tulong-selection-and-monitoring.md` 只保留最小索引和当前已确认主规则。若旧的单日 validation notes 已被更新的多日共同结论吸收，应删除旧 notes 并同步清理 `SKILL.md` 的 reference 分工，避免 stale 策略继续污染后续判断。
...
0601D3 与 0602D3 连续复盘后形成一个候选方向：D3 可能需要在现有低吸观察池之外，单独记录“强势延续雷达”，用于复盘 D1 全量底池中未入 watch 但 D3 直接强势延续的样本。

该方向暂不作为主规则细节固化；具体策略、触发候选、样本依据与后续优化记录维护在：`reports/reviews/tulong_d3_strategy_reviews/0601_0602D3_common_strategy_conclusion_20260602.md`。

## D3 盘中人工补票与“强提醒追高票”例外

默认 D3 观察区仍坚持“低吸可执行性 AND 强势潜质”，但盘中用户可能基于板块强度、分时观察或漏筛纠正，要求把某只未在自动窄化池里的股票补入 `MMDDD3`，甚至明确指定为“强提醒追高票”。这类处理要作为**人工覆盖观察口径**，不要混同为普通低吸 D3。
...
- 行情拉取/事件检测：每 1 分钟；
- 微信提醒释放：每 5 分钟一个窗口；窗口内先记录到本地 `pending_alerts`，到 5 分钟边界再投递；
- 同一只股票在 5 分钟窗口内多次触发时，微信只发优先级最高的一条；所有分钟级事件仍写入本地 JSONL/快照/日志，供收盘复盘使用；
- 快照生成/提醒：每 15 分钟生成；默认只写本地，不推微信；
- 收盘复盘：15:10 左右；
- 有单股事件时先入队，不等快照点；到微信释放窗口时事件优先，快照仍只落本地；
- 非 5 分钟释放点且无 stdout 时静默，避免微信/iLink 限流；
