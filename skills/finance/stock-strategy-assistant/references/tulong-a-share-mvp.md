...
### D4/D5 退出（验证期已移除，仅作历史记录）

> 验证期调整（2026-05-30）：以下 D4/D5 退出规则属于早期口头底稿，已不作为有效策略，**不再预设、不再执行**，以免约束后续策略制定。买入后的退出/加仓规则待积累持仓生命周期数据并验证后再制定。
>
> 历史底稿（保留备查，勿当作当前规则）：D4 曾设想“高开超 4% 止盈/减仓、平开震荡拉高卖出、跌破 D1_support 退出”；D5 曾设想“除非极强行情否则窗口结束退出”。
