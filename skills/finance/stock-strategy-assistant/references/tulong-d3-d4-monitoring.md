...
- 确认或更新次日 `MMDDD3观察区` 时，必须先读取已确认/已落盘的 `MMDDD4_position_*_YYYYMMDD_HHMMSS.csv`。如果同一只票同时满足新 D3 扫描和既有 D4 持有区，默认保留在 D4 持有区，不重复放入 D3；除非用户明确要求双标签跟踪。
...
默认 D3 候选区仍坚持“低吸可执行性 AND 强势潜质”，但盘中用户可能基于板块强度、分时观察或漏筛纠正，要求把某只未在自动窄化池里的股票补入 `MMDDD3`，甚至明确指定为“强提醒追高票”。这类处理要作为**人工覆盖观察口径**，不要混同为普通低吸 D3。
...
当用户确认次日 `MMDDD3` 观察/持仓或 `MMDDD4` 持有区名单时，不能只在聊天中复述“已确认”。如果用户明确说“新会话还要继承/明天监控/是否落盘”，必须立即核对或生成对应源文件：
