...
用于 A 股“屠龙战术”D3/D4 流转与盘中提醒：D3 只监控当天候选区；D4 只保留已买入后的持有区，不再设置未买入观察区或强势雷达区。核心目标是把提醒做成“可执行区提醒”，而不是状态播报。
...
- `0526D3持仓 → 0527D4持有区`：D3 当天已经买入的持仓，次日进入 D4 持有区；D4 用于可卖日持仓管理。
- D4 不再从未买入观察票或强势票里生成观察区；D3 未买入票即使走强，次日也不进入 D4。
- D3/D4 源文件落盘时，文件名尾部必须带年月日+时分秒时间戳，并显式包含分区：`MMDDD3_watch_<原因>_YYYYMMDD_HHMMSS.csv`、`MMDDD3_position_<原因>_YYYYMMDD_HHMMSS.csv`、`MMDDD4_position_<原因>_YYYYMMDD_HHMMSS.csv`。其中 `<原因>`（如 `scan`、`manual_review`、`position_rollover`）只用于人类理解这次改动来源；监控池选择数据源时按 `MMDDD3/MMDDD4 + pool_type + YYYYMMDD_HHMMSS` 取每类最新。不要再生成或依赖仅带 `HHMMSS` 的新文件；看文件名尾部时间戳就应能判断哪个更新。找不到新格式文件时必须失败提醒，而不是静默使用旧格式。
- `0526D3未走强 → 0527删除`：前一日 D3 没走强，次日不再留在候选区。
- `0527新D3候选区`：5 月 27 日新生成的当天 D3 票。

D3 候选区只放当天 D3。前一交易日已经是 D3 的未买入观察票，次日不继续留在 D3，也不流转到 D4；只有 D3 已买入持仓，次日才流转为 D4 持有区。
...
- 夜间确认次日 `MMDDD3观察区` 时，必须先读取已确认/已落盘的 `MMDDD4_position_*_YYYYMMDD_HHMMSS.csv`。如果同一只票同时满足新 D3 扫描和既有 D4 持有区，默认保留在 D4 持有区，不重复放入 D3；除非用户明确要求双标签跟踪。
...
- 次日 D3 与次日 D4 同时存在时，用户可见结论要分开写：先说明 D4 持有区已落盘状态，再单独给 D3 初选/建议名单，避免把 D3 观察与 D4 持有混成一个池。
...
规则演进原则：屠龙 D3/D4 规则会随着复盘持续调整。若同一规则链路内前后描述出现冲突，优先采用后续新确认的规则；确认加入的新规则必须回写到原有 `D1过滤规则` 或 `D2过滤规则` 对应条目里，直接修改原规则，不保留“旧规则 + 新补充”的并列层。
...
这些判断属于生成 `0527D3候选区` 的 D1/D2 过滤规则，不属于 D4 持有区的监控指标。

### D3 夜间人工窄化复核清单

当机械扫描给出 D3 初选后，夜间与用户确认 `MMDDD3观察区` 时，必须再做一轮更窄的人工复核，不要直接落盘机械排序靠前的名单：

1. **先查重**：读取已确认/已落盘的同日 D4 持有区；同一只票默认保留在 D4，不重复放入 D3，除非用户明确要求双标签。
...
默认 D3 候选区仍坚持“低吸可执行性 AND 强势潜质”，但盘中用户可能基于板块强度、分时观察或漏筛纠正，要求把某只未在夜间池里的股票补入 `MMDDD3`，甚至明确指定为“强提醒追高票”。这类处理要作为**人工覆盖观察口径**，不要混同为普通低吸 D3。
...
3. 动作：等、可小仓观察、放弃、不追；若已买入则次日转 D4 持有区管理；
...
D4 只保留 **D4持有区**，不再设置未买入 watch，也不再把 D4 命名为强势雷达区。进入 D4 的前提是 D3 已买入并形成持仓；D4 的任务是可卖日持仓管理，而不是寻找新的参与机会。

核心区别：

- D3观察区：看买点区、回收/观察价、止损/失效位；
- D3持仓区：当天已买入，受 T+1 限制，重点是持仓风险标记和隔夜风险；
- D4持有区：已进入可卖日，重点是止损/止盈/减仓、是否继续持有、是否允许加仓、尾盘是否留仓。

D4 持有区触发原则：

1. **持仓风控优先**：先看是否跌破失败线、止损位或关键承接位，再看是否继续持有。
2. **止盈/减仓要前置**：若冲高但不能封住、回落破关键价，提醒应直接给出减仓/止盈/降低仓位口径。
3. **加仓不是默认动作**：只有回踩承接不破并重新走强，或突破确认后仍有风险收益比，才提示可加仓观察。
4. **尾盘留仓单独判断**：尾盘不强、接近失败线或放量回落，应提示降低仓位或不留弱仓。

D4 持有区单股提醒模板：

```text
0527D4｜代码 名称｜行业｜持有区/止损风险/止盈减仓/继续持有/可加仓观察
现价 xx（+x.xx%）｜成本 xx｜可卖数量 xxx｜失败线 xx
动作：跌破失败线处理 / 冲高不能封住减仓 / 回踩不破可继续持有
参考：今日高点xx / 前高xx / 成交额x.xx亿｜HH:MM
```
...
后续用户可见概念统一使用“观察区”，不再使用“候选区”；工程字段使用 `pool_type=watch` 表示未买入观察，`pool_type=position` 表示已买入持仓。D4 只使用 `position`，不使用 `watch`。详细概念模型见 `references/tulong-position-pool-concepts.md`。

核心口径：

- `D3观察区`：未买入，寻找买点/回收/失效；
- `D3持仓区`：已买入，管理当天持仓风险。因 A 股 T+1，当天不能卖，只能加仓观察/风险标记/记录隔夜风险；提醒中避免“卖出、减仓、止盈”等动作词；
- `D4持有区`：已买入，进入可卖日，可以卖也可以加；提醒重点是止损/止盈/减仓、是否继续持有、是否回踩承接可加仓、尾盘是否留仓。
...
2. **T 日开盘前切换实际监控池**
  ...
  - active CSV 必须包含 `stage`、`pool_type`、`source_file` 字段，用于区分 `0528D3/watch`、`0528D3/position` 与 `0528D4/position` 等来源；
...
08:50        开盘前切换实际监控池：写入今日 D3 watch/position 与 D4 position，重置状态
...
1. `preopen_rotate_watchlist`
  - 查找今日 `MMDDD3_watch_*_YYYYMMDD_HHMMSS.csv`、`MMDDD3_position_*_YYYYMMDD_HHMMSS.csv`、`MMDDD4_position_*_YYYYMMDD_HHMMSS.csv`，每类按显式 `YYYYMMDD_HHMMSS` 取最新；中间 `<原因>` 只作为改动来源，不参与优先级；自 0528 起不再兜底无时间戳旧名，找不到任何新格式文件就失败提醒；
...
- 如果当日没有 D4 position 来源文件，守门任务可记录 warning，但不要因此阻断 D3 切池；只有存在真实持仓时才需要补 D4 持有区流转。
- 调度顺序应早于盘中 watchdog：08:50 切池、09:05 守门、09:25 后盘中有效。

## 观察区/持仓区确认后的即时落盘纪律

当用户在夜间复盘中确认次日 `MMDDD3` 观察/持仓或 `MMDDD4` 持有区名单时，不能只在聊天中复述“已确认”。如果用户明确说“新会话还要继承/明天监控/是否落盘”，必须立即核对或生成对应源文件：

1. 每个分区都要有独立源文件：
  - D3观察区：`MMDDD3_watch_*_YYYYMMDD_HHMMSS.csv`；
  - 持仓/持有区：`MMDDD3_position_*_YYYYMMDD_HHMMSS.csv` / `MMDDD4_position_*_YYYYMMDD_HHMMSS.csv`。
2. 不要把“实际 active 池里已有持仓”误判为“持仓源文件也已落盘”。必须分别检查 D3 watch、D3 position、D4 position。
3. 不要再生成或依赖 `MMDDD4_watch_*` / `D4_strong_radar_*` 文件；D4 只接受 `MMDDD4_position_*`。
...
4. 检查 active CSV 的 `stage`、`pool_type`、`source_file` 字段，确认每只票来自 `D3观察区(watch)`、`D3持仓区(position)` 或 `D4持有区(position)`；
