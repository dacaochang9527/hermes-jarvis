...
> 本文继续保留的部分：D1/D2/D3 选股过滤、观察 vs 持仓的事实区分、T+1 可卖性、开盘前换日切池/守门等**工程事实**。凡“D4 持有区/D5 应如何卖、如何加、如何止损止盈”的动作规则均已中性化为占位，待数据验证后另行制定。
...
验证期内不预设买入后的退出/加仓策略。原“D4 持有区规则”（止损/止盈/减仓/加仓/留仓的触发原则与提醒模板）已整体移除，避免约束后续策略制定。
...
- `持仓（已买入）`：`pool_type=position`。仅按事实区分 T+1 可卖性——买入当天不可卖、次日起可卖；验证期不预设买入后的卖出/加仓/止盈止损动作规则。

可卖性由 `entry_date` 客观派生，不再靠 `stage` 是 D3 还是 D4 来判断。
