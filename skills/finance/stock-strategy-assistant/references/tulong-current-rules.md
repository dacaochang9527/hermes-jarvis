...
这里的低吸可执行性表示必须有舒服的参与/风控点，不代表只做水下低吸。板块强度参考分归入“强势潜质”侧，只作为排序、active/radar 分层和盘中回收确认的参考权重，不替代个股结构、买点区和失效线纪律。D3 至少分三类看：
...
- active 主池 4–6 只（正式D3观察池，可参与，优先 D3 当天可执行性、买点/止损清楚、安全垫足、风险收益比舒服）；
- radar 强势雷达 4–6 只（强势但慎追，只观察不默认参与；不写入正式 active 监控池）；
- 高频/强提醒不超过 4–5 只；
- 其余观察区票只进低频快照。
...
正式输出建议分层，不把所有强势延续票直接放入 active：

```text
active：正式 D3 主池，优先低吸可执行性 AND 强势潜质，买点/止损清楚、安全垫足，默认 4–6 只；
radar：强势雷达/备选池，D2 强势延续但 D3 买点不舒服、追高风险大、成交拥挤或高价，默认 4–6 只；
backup：低频备选；
exclude：剔除，仅在报告中说明原因。
```

D2 高强度延续不能自动等同于进入 active。D2 涨停/接近涨停但缺少充分回落、买点区距离现价不舒服、或 D3 更像追高时，应优先进入 radar 而不是 active；当前规则下 D2 强势延续默认先进 radar，盘中二次回收确认后再看是否参与。标准规则负责“不漏强”，但正式 active 必须用“可参与性 AND 强势潜质”二次分层。

风险标记必须影响排序/分层，而不只是写在 note 中：D1 炸板 >=4 次、尾盘封板、高价股（尤其 80/100 以上）、成交额拥挤（如 15–20 亿以上）、D2 过强无回落、近期明确股东减持/拟减持、控股股东/实控人协议转让、股权质押、密集股票交易风险提示或异常波动公告，都应触发降权、降层或进入 radar/exclude。当前 selection 口径：D2 强势延续只保留少量结构分，不再直接高加分；D1 尾盘封板、D1 炸板 >=4、成交额 >20 亿、价格 >100 默认带 radar_only；成交额 15–20 亿、高价 80–100、炸板偏多做实质降权。

板块强度参考分用于补足“指数强但个股不共振”的二次校验：先用同批通过 D2过滤规则候选的 `industry` 聚集度、行业内 D2 平均涨跌幅、行业内 D2 强势数量计算 `sector_strength_score`，上限 +8、下限 -6；同一行业候选聚集、D2 均涨、行业内多只强势给加分，行业内 D2 均跌给降权并标记“板块弱势”。该分数写入 `sector_strength_score` / `sector_strength_note`，进入总评分和报告说明，但不作为硬过滤；强板块但买点不舒服仍进 radar，弱板块但个股结构极好也可保留观察。
...
输出时直接给出自动窄化后的观察名单，并列出未输出原因。落盘文件统一使用 `MMDDD3_watch_scan_YYYYMMDD_HHMMSS.csv`，并包含 `pool_subtype` 字段区分 active / radar / backup / exclude。active 默认容量 4–6 只，radar 默认 4–6 只；开盘前切池只把 `pool_subtype=active` 写入正式 active 监控池，radar 只留在报告/CSV 中作为强势观察，不默认触发正式提醒。
...
```text
D3 active交易       # 当日 active 主池内发生的买卖
D3 radar观察但未买   # radar 仅观察，若发生交易也不混入 active 胜率
HOLD管理            # 旧仓、T 操作、买入后转 HOLD 的持仓管理
策略外交易          # 不属于当日 D3 watch/HOLD 的个股交易
ETF                 # ETF/基金单独统计，不进入屠龙 D3 胜率
```
...
默认规则边界：用户只说“生成某日 D3 / 生成 MMDDD3”时，默认使用标准主规则生成标准池（`MMDDD3_watch_scan_YYYYMMDD_HHMMSS.csv`），不要默认使用朋友规则、`new_strategy` 或 `new_strategy_full`。只有用户明确说“按朋友规则 / new_strategy / new_strategy_full / 昨天那10只口径 / 做对照”，才额外生成外部策略结果或做差异对照。

策略调整边界：当用户说“调整策略规则”但未明确要求修改朋友规则时，优先把复盘确认有效的部分吸收到标准主规则；朋友规则先保持为外部策略输入/对照实验。不要因为某天朋友规则表现较好，就把它静默设成默认主规则。
