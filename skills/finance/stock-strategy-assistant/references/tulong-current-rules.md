...
单日复盘中发现的强弱样本或规则修正想法，先作为候选结论落到 `reports/reviews/` 的当日复盘报告，不要立即改写当前主规则。只有经用户明确确认或多日样本共同验证后，才按“规则变更同步协议”同步到规则函数、selection/runtime、运行手册和测试。

示例候选类型：

- active 内 D2 量比 2–3 倍样本在 D3 低开弱承接时，是否需要更快降级；
- radar 中 D2 缩量过弱但 D3 高开放量强攻的标的，是否需要盘中二次提升机制；
- 买点区提醒是否要进一步区分“进入买点区后转强”和“进入买点区后继续破位”。
