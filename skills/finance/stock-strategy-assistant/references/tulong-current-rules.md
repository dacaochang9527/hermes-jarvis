...
每次生成或更新 D3 初选后，必须立即自动窄化。后续人工复核只负责修正自动窄化结果，不作为唯一窄化步骤。
...
- 人工分时校正：若用户根据分时图纠正接口字段，应修正判断并在落盘 note 中保留原因。

输出时直接给出自动窄化后的观察名单，并列出未输出原因。用户确认人工修正名单后，再落成 `MMDDD3_watch_manual_review_YYYYMMDD_HHMMSS.csv`。
...
- D3 watch 观察池：经过 D2确认和 D3 自动/人工窄化；
- HOLD 持仓：买入后的事实层。
...
盘中用户可能基于板块强度、分时观察或漏筛纠正，把未在自动窄化池里的股票补入 `MMDDD3`，甚至指定为“强提醒追高票”。处理纪律：

- 先核验基础 D3 条件：D1 首板、D2 未破 D1 支撑、D2 量能不超过 3 倍、主板 10cm、无 ST/退市风险；
- 明确这是人工覆盖观察口径，不混同为普通低吸 D3；
- `note` 写清 `manual_add_strong_chase` 或对应人工原因；
- 点位不要沿用低吸模板，`trigger_price` 优先用 D2 高点/前高/盘中高点等强势确认位，`invalid_price` 使用关键承接位或用户认可的失败线；
- 同步更新人工源文件和实际 active 池，并清理该代码旧状态后验证提醒文本。
