...
- active 主池 6–8 只（正式D3观察池，可参与，优先 D3 当天可执行性、买点/止损清楚、风险收益比舒服）；
...
```text
active：正式 D3 主池，优先低吸可执行性 AND 强势潜质，买点/止损清楚，默认 6–8 只；
radar：强势雷达/备选池，D2 强势延续但 D3 买点不舒服、追高风险大、成交拥挤或高价，默认 4–6 只；
backup：低频备选；
exclude：剔除，仅在报告中说明原因。
```

D2 高强度延续不能自动等同于进入 active。D2 涨停/接近涨停但缺少充分回落、买点区距离现价不舒服、或 D3 更像追高时，应优先进入 radar 而不是 active。标准规则负责“不漏强”，但正式 active 必须用“可参与性 AND 强势潜质”二次分层。

风险标记必须影响排序/分层，而不只是写在 note 中：D1 炸板 >=4 次、尾盘封板、高价股（尤其 80/100 以上）、成交额拥挤（如 15–20 亿以上）、D2 过强无回落，都应触发降权、降层或进入 radar。当前 selection 口径：D2 强势延续只保留少量结构分，不再直接高加分；D1 尾盘封板、D1 炸板 >=4、成交额 >20 亿、价格 >100 默认带 radar_only；成交额 15–20 亿、高价 80–100、炸板偏多做实质降权。
...
输出时直接给出自动窄化后的观察名单，并列出未输出原因。落盘文件统一使用 `MMDDD3_watch_scan_YYYYMMDD_HHMMSS.csv`，并包含 `pool_subtype` 字段区分 active / radar / backup / exclude。active 默认容量 6–8 只，radar 默认 4–6 只；开盘前切池只把 `pool_subtype=active` 写入正式 active 监控池，radar 只留在报告/CSV 中作为强势观察，不默认触发正式提醒。
