#!/usr/bin/env bash
# Gateway/plugin entry for /cursor-run — switch model and submit Composer prompt.
set -euo pipefail

usage() {
  cat >&2 <<'USAGE'
Usage: dispatch_cursor_run.sh [--window WORKSPACE] MODEL prompt...

Examples:
  dispatch_cursor_run.sh gpt-5.5 Review gateway.log for errors
  dispatch_cursor_run.sh --window startell composer Fix the failing test
USAGE
}
...
if [[ ${#ARGS[@]} -lt 1 ]]; then
  echo "error: cursor-run 需要 MODEL" >&2
  usage
  exit 2
fi
...
if [[ -n "${CURSOR_AUTOMATION_PROMPT:-}" ]]; then
  ...
  echo "error: cursor-run 需要 MODEL 和 prompt" >&2
  usage
  exit 2
fi
