...
- **Feishu group setup and allowlists**: For adding Hermes to Feishu groups, discovering group `chat_id`, testing active group sends, listing members, and restricting which users can trigger the bot, see `references/feishu-gateway-groups.md`. If group sends work by raw `feishu:<chat_id>` but the group is absent from `send_message list`, continue using the raw target and treat the list as a convenience registry, not the source of truth.
