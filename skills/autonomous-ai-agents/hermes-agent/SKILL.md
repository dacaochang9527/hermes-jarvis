...
For producing a user-approved final write draft after REVIEW decisions, see `references/openclaw-memory-migration-final-draft-case.md`. It covers nonexistent Hermes memory files, fresh-content wording, handling user-removed scope, and keeping confirmed timestamp/check-in preferences without copying old OpenClaw channel routing.
...
- **Weixin media attachment missing**: Treat text delivery and media delivery as separate stages. Before claiming an image/file was sent, verify the referenced `MEDIA:` path exists on the gateway host and points to the final artifact rather than a provider/container-only temporary path such as `/mnt/data/...`. Send through `send_message` using the valid absolute host path, require a successful return with `message_id`, and then inspect the newest gateway log if the user still cannot see it. Log lines such as `Sending image` only prove the attempt began; `Failed to send image`, `send_document failed`, or `No such file or directory` mean delivery failed. A later text-only response does not retry the attachment—include `MEDIA:<absolute_path>` again explicitly. See `references/weixin-media-delivery-verification.md` for the layered checklist and failure interpretation.
- **Feishu group setup and allowlists**: For adding Hermes to Feishu groups, discovering group `chat_id`, testing active group sends, listing members, and restricting which users can trigger the bot, see `references/feishu-gateway-groups.md`. If group sends work by raw `feishu:<chat_id>` but the group is absent from `send_message list`, continue using the raw target and treat the list as a convenience registry, not the source of truth. Group `@所有人` / Feishu `@_all` should not count as a bot mention for reply admission; only explicit bot mentions or configured command triggers should start an agent turn. When debugging this, inspect `gateway/platforms/feishu.py::_mentions_self` and keep tests around `TestGroupMentionAtAll` aligned with the no-reply behavior.
- **Feishu `@所有人` noise control**: Group messages containing only Feishu's `@_all` / `@所有人` should not count as an explicit bot mention. If the bot replies to broad group announcements, inspect `gateway/platforms/feishu.py::_mentions_self`; keep `@_all` out of self-mention detection, update `tests/gateway/test_feishu.py::TestGroupMentionAtAll`, then restart the gateway so the adapter reloads.
